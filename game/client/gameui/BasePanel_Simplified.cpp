...
ConVar vgui_message_dialog_modal_client( "vgui_message_dialog_modal_client", "1", FCVAR_ARCHIVE );
...
static ConCommand gamemenucommand_client( "gamemenucommand_client", CC_GameMenuCommand, "Issue game menu command. (Doesn't work)", 0, CC_GameMenuCompletionFunc );
...
void CBasePanel::UpdateBackgroundState()
{
	...
	if ( GameUI().IsConsoleUI() )
	{
		if ( !m_ExitingFrameCount && !m_bLevelLoading && GameUI().IsInLevel() )
		{
			// paused
...
{
	if ( !Q_stricmp( command, "OpenOptionsDialog2" ) )
	{
		if ( !GameUI().IsConsoleUI() )
		{
			OnOpenOptionsDialog();
...
void CMessageDialogHandler::CloseAllMessageDialogs()
{
	for ( int i = 0; i < MAX_MESSAGE_DIALOGS; ++i )
	{
		...
		{
			vgui::surface()->RestrictPaintToSinglePanel(NULL);
			if ( vgui_message_dialog_modal_client.GetBool() )
			{
...
static ConCommand gameui_show_dialog_client( "gameui_show_dialog_client", CC_GameUIShowDialog, "Show an arbitrary Dialog.", 0 );
...
static ConCommand gameui_hide_dialog_client( "gameui_hide_dialog_client", CC_GameUIHideDialog, "asdf", 0 );
...
static ConCommand refresh_options_dialog_client( "refresh_options_dialog_client", RefreshOptionsDialog, "Refresh the options dialog.", 0 );
