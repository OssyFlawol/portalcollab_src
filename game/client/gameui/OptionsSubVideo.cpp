...
public:
	CGammaDialog( vgui::VPANEL hParent ) : BaseClass( NULL, "OptionsSubVideoGammaDlg" )
	{
		...
		LoadControlSettings( "resource/OptionsSubVideoGammaDlg2.res" );
...
public:
	COptionsSubVideoAdvancedDlg( vgui::Panel *parent ) : BaseClass( parent , "OptionsSubVideoAdvancedDlg" )
	{
		...
		LoadControlSettings( "resource/OptionsSubVideoAdvancedDlg2.res" );
		MoveToCenterOfScreen();
		SetSizeable( false );
	...
	void MarkDefaultSettingsAsRecommended()
	{
		...
		int nD3D9Ex = pKeyValues->GetInt( "ConVar.mat_disable_d3d9ex", 1 );
		...
		SetComboItemAsRecommended( m_pD3D9Ex, nD3D9Ex );
	...
	virtual void ApplyChanges()
	{
		...
		ApplyChangesToConVar( "mat_disable_d3d9ex", m_pDXLevel->GetActiveItem() );
	...
	virtual void OnResetData()
	{
		...
		m_pD3D9Ex->ActivateItem( mat_disable_d3d9ex.GetInt() );
...
{
	FILE *fp = FOpenGameHDFile(NULL, "rb" );
	if ( fp )
	{
		fclose(fp);
		return true;
...
void COptionsSubVideo::SetUseHDContent( bool bUse )
{
	if ( bUse )
	{
		FILE *fp = FOpenGameHDFile( NULL, "wb+" );
		if ( fp )
		{
...
void COptionsSubVideo::OpenAdvanced()
{
	if ( !m_hOptionsSubVideoAdvancedDlg.Get() )
	{		
		//m_hOptionsSubVideoAdvancedDlg = new COptionsSubVideoAdvancedDlg( BasePanel()->FindChildByName( "OptionsDialog2" ) ); // we'll parent this to the OptionsDialog directly
		
		//Use this hack instead:
		vgui::Panel *parent = (vgui::Panel *)NULL;		
		m_hOptionsSubVideoAdvancedDlg = new COptionsSubVideoAdvancedDlg( parent );

		if (OptionsDialog() == NULL)
			Warning("Couldn't find OptionsDialog!!\n");

		m_hOptionsSubVideoAdvancedDlg->SetParent(OptionsDialog());
...
public:
   COptionsSubVideoThirdPartyCreditsDlg( vgui::VPANEL hParent ) : BaseClass( NULL, NULL )
   {
      ...
      SetSize( 500, 200 );
      LoadControlSettings( "resource/OptionsSubVideoThirdPartyDlg2.res" );
      MoveToCenterOfScreen();
      SetSizeable( false );
      SetDeleteSelfOnClose( true );
   }
